...
import { shifts, employeeRoles, productTypes, storageRacks, furnaces } from "./db_constants";
...
export const populate = async () => {
    try {
        const allemployeeRoles = () => { return employeeRoleModel.find({}, { name: 1, _id: { "$toString": "$_id" } }).exec() }
        const allfurnaces = () => { return furnaceModel.find({}, { name: 1, _id: { "$toString": "$_id" } }).exec() }
        const allproductTypes = () => { return productTypeModel.find({}, { name: 1, _id: { "$toString": "$_id" } }).exec() }
        const allStorageRacks = () => { return storageRackModel.find({}, { rackNumber: 1, _id: { "$toString": "$_id" } }).exec() }
        const allShifts = () => { return shiftModel.find({}, { name: 1, _id: { "$toString": "$_id" } }).exec() }
        const assignVariable = async () => {
            try {
                ...
                return [allemployeeRolesArr, allfurnacesArr, allproductTypesArr, allStorageRacksArr, allShiftsArr]
