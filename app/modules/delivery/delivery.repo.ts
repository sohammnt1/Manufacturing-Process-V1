  ...
  deliveryModel.aggregate([
    {
      $match: {
        $and: [
          { deliveryExecutiveId: new ObjectId(id) },
...
const attemptDelivery = (updated_data: IDelivery) =>
...
const deleteOne = (deliveryId: string) =>
  deliveryModel.updateOne({ deliveryId: deliveryId }, { deleted: true });
